...
class TextChunker:
    ...
    def _split_by_separator(self, text: str, start_offset: int, separator_pattern: str) -> List[tuple[str, int]]:
    ...
    def _recursive_split(self, text: str, start_offset: int = 0, separator_index: int = 0) -> List[tuple[str, int]]:
        ...
        for part_text, part_offset in parts:
            if len(part_text) > self.config.chunk_size:
                if last_separator:
                    ...
                    result.extend(self._recursive_split(part_text, part_offset, separator_index + 1))
            else:
                result.append((part_text, part_offset))
                
        return result
    
    def _character_split(self, text: str, start_offset: int = 0) -> List[tuple[str, int]]:
        ...
        while start < len(text):
            ...
            if end < len(text):
                ...
                sentence_end = text.rfind(".", start + self.config.minimum_chunk_size, end)
                if sentence_end != -1 and sentence_end > start + self.config.minimum_chunk_size:
                    end = sentence_end + 1
                else:
                    # Look for paragraph breaks
                    paragraph_end = text.rfind("\n\n", start + self.config.minimum_chunk_size, end)
                    if paragraph_end != -1 and paragraph_end > start + self.config.minimum_chunk_size:
                        end = paragraph_end + 2
                    else:
                        # Look for line breaks
                        line_end = text.rfind("\n", start + self.config.minimum_chunk_size, end)
                        if line_end != -1 and line_end > start + self.config.minimum_chunk_size:
                            end = line_end + 1
                        else:
                            # Look for word boundaries
                            word_end = text.rfind(" ", start + self.config.minimum_chunk_size, end)
                            if word_end != -1 and word_end > start + self.config.minimum_chunk_size:
                                end = word_end + 1
    ...
    def chunk_document(self, document_id: str, content: str, source_file: str) -> List[Chunk]:
